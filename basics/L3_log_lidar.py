...
try:
    while True:
        myVector = getNearest(lidarsensor.get()) 
        log.tmpFile(myVector[0],"distance")  
        log.tmpFile(myVector[1],"angle")     
        
        x_comp = (myVector[0])*math.cos(myVector[1])
        y_comp = (myVector[0])*math.sin(myVector[1])

        log.tmpFile(x_comp,"xcomp")  
        log.tmpFile(y_comp,"ycomp") 
                                                 # call the function which utilizes several functions in this program
        print("\n The nearest object (m,deg):\n", myVector[0])     # print the result
        time.sleep(0.5)
