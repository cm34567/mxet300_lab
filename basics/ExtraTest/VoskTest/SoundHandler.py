...
class VoskKeywordProcessor:
    ...
    def __init__(self, model_path, audioIn = -1, audioOut = -1):
        self.actionQueue = queue.Queue()
        self.triggerKeywords = {"rex", "rec", "recs", "wrecks", "rats", "racks", "wracks", "what", "what's", "let's", "brakes"}
        self.actionKeywords = {
            self.Actions.SPIN: {"spin", "spun", "spine", "spoon"},
            self.Actions.BARK: {"bark", "bar", "mark", "park", "dark", "shark", "lark"},
            self.Actions.STOP: {"stop", "stoop", "step", "top", "stob", "stock", "start", "stuff"}
        }
        self.lastRecognizedText = ""
        self.sentenceTriggered = False

        self.vosk_rec = VoskRecognizer(
            model_path,
            16000,
            16000,
            audioIn,
            None,
            self.processText)
        self.bh = BarkHandler(audioOut)

        
        self._keepSpin=False
        self._wheelspeed=np.array((0,0))
    ...
    def process(self):
        ...
        if(action == self.Actions.BARK):
            self.bh.play_bark()
            print("barking")
        elif(action==self.Actions.SPIN):
            self._wheelspeed=ik.getPdTargets([0,0.5])
            self._keepSpin=True
            print("spinning")
        ...
        if (self._keepSpin):
            sc.driveOpenLoop(self._wheelspeed)
        else:
...
class BarkHandler:
    def __init__(self, device=-1):
        ...
        volume_factor = 100.0
        self.bark_data = np.clip(self.bark_data * volume_factor, -1.0, 1.0)
        self.device = device

    def play_bark(self):
        """Play bark in a separate thread without stopping other sounds."""
        def _play():
            sd.play(self.bark_data, self.bark_fs, device=self.device)
            sd.wait()  # wait until this playback finishes
